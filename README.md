...
右上角的“浅色／深色”按钮可切换整站配色。首次访问跟随系统；手动选择会保存在当前浏览器，刷新和进入其他文章时继续生效。同一网站的多个标签页也会同步选择。若浏览器禁止本地存储，仍能在当前页面切换，但无法跨页面记忆。

配色在 `source/css/blog.css` 中维护，切换逻辑在 `source/js/theme.js`。主题脚本在页面头部执行，优先应用已保存的选择，减少刷新时的配色闪烁；CSS 和脚本引用包含内容摘要，发布后可加载更新版本。
